...
export async function askOpenAIDirect(input: {
  ...
  const response = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({
      model: input.model,
      instructions: `${input.persona.systemPrompt}\n\nReturn ONLY JSON in this shape: {"speech":"spoken response","intent":"chat|play_music|play_youtube|media_control|translate|persona|agent|system|unknown","actions":[]}. Keep speech natural and short for voice. Use actions only when the user asked for a device/media/config action.`,
      input: `${transcript}\nUser: ${input.text}\nAGA JSON:`,
    }),
  });
