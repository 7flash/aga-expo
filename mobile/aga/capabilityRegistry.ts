...
export const BUILTIN_CAPABILITY_TOOLS: readonly RealtimeToolDefinition[] = [
  ...
  {
    type: 'function',
    name: 'learn_user_scenario',
    description: 'Create a learned voice scenario from the user saying “when I say X, do Y”. Use only when the user explicitly teaches AGA a behavior.',
    parameters: schema({
      text: { type: 'string', description: 'The full user utterance containing the trigger and desired behavior.' },
    }, ['text']),
  },
  {
    type: 'function',
    name: 'accept_learned_routine',
    description: 'Accept the top proposed learned routine or a routine matching spoken text. This enables future proactive offers.',
    parameters: schema({ match: { type: 'string' } }),
  },
  {
    type: 'function',
    name: 'dismiss_learned_routine',
    description: 'Dismiss the top proposed learned routine or a routine matching spoken text.',
    parameters: schema({ match: { type: 'string' } }),
  },
  {
    type: 'function',
    name: 'summarize_learned_patterns',
    description: 'Summarize learned habits, routines, emotional patterns, and helpful techniques for the user.',
    parameters: schema({ topic: { type: 'string' } }),
  },
  {
    type: 'function',
    name: 'soundscape_companion',
    description: 'Start a lightweight local soundscape under conversation or guided work. Prefer this over YouTube for ambient music.',
    parameters: schema({
      query: { type: 'string' },
      useCase: { type: 'string', enum: ['conversation', 'guided', 'sleep', 'focus', 'conflict'] },
  ...
  },
  {
    ...
    description: 'Control the active guided session: pause, resume, next, deeper, skip, repeat, or end.',
    parameters: schema({ command: { type: 'string', enum: ['pause', 'resume', 'next', 'deeper', 'skip', 'repeat', 'end'] } }, ['command']),
  },
  {
    type: 'function',
...
export function buildTurnContextBlock(prefs: Preferences | null) {
  ...
  const parts = [
    ...
    'If the user teaches “when I say X, do Y”, call learn_user_scenario. If AGA notices repeated helpful timing, propose a routine and ask for consent before enabling it.',
    'For ambient background audio, prefer soundscape_companion/local ambient over YouTube unless the user explicitly asks for YouTube.',
    ...profileLines,
  ];
