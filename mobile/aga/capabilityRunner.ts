...
import { controlGuidedStatefulSession, guidedCueToSpeech, startGuidedStatefulSession } from '../skills/guidedSessionMachine';
...
import { observeHabitSignal } from '../learning/habitTracker';
import { saveUserTaughtScenario } from '../learning/scenarioLearner';
import { rememberSoundscapeUse, resolveSoundscapeCompanion } from '../media/soundscapeCompanion';
import {
  executeRemoteTool,
...
export function createCapabilityRunner(ctx: CapabilityRunnerContext) {
  ...
  const handlers: Record<string, CapabilityHandler> = {
    ...
    play_youtube: async ({ query, forceYouTube }) => {
      ...
      if (!forceYouTube && !explicitYouTube) {
        ...
        if (ambient) {
          ...
          await observeHabitSignal({ kind: 'music', label: ambient.title, context: { query: q, ambientKind: ambient.kind }, confidence: 0.58 });
          await ctx.refresh();
    ...
    learn_user_scenario: async ({ text }) => {
      const skill = await saveUserTaughtScenario(String(text || ''));
      if (!skill) return 'I can learn that, but say it like: AGA, when I say wind down, start breathing and rain sounds.';
      await logEvent('skill.user_taught', `${skill.id}: ${skill.label}`);
      await ctx.refresh();
      return `I learned that scenario: ${skill.label}.`;
    },
    accept_learned_routine: async ({ match }) => {
      const routine = await acceptProposedRoutine(match ? String(match) : undefined);
      await ctx.refresh();
      if (!routine) return 'I could not find a proposed routine to accept.';
      await logEvent('routine.accepted', `${(routine as any).id}: ${(routine as any).title}`);
      return `Accepted. I will gently offer ${(routine as any).title} when the pattern appears, and you can dismiss it any time.`;
    },
    dismiss_learned_routine: async ({ match }) => {
      const routine = await dismissProposedRoutine(match ? String(match) : undefined);
      await ctx.refresh();
      if (!routine) return 'I could not find a proposed routine to dismiss.';
      await logEvent('routine.dismissed', `${(routine as any).id}: ${(routine as any).title}`);
      return `Dismissed ${(routine as any).title}.`;
    },
    summarize_learned_patterns: async ({ topic }) => {
      const patterns = await listScenarioPatterns(6).catch(() => [] as any[]);
      const routines = await listDueAcceptedRoutines(new Date(), 4).catch(() => [] as any[]);
      const found = await searchMemories(topic ? String(topic) : 'Helpful technique emotional pattern routine', 6).catch(() => [] as any[]);
      const lines = [
        patterns.length ? `Observed patterns: ${patterns.map((p: any) => `${p.label} (${Math.round(Number(p.confidence || 0) * 100)}%)`).join('; ')}` : '',
        routines.length ? `Accepted routines ready now: ${routines.map((r: any) => r.title).join('; ')}` : '',
        found.length ? `Relevant memories: ${found.map((m: any) => m.text).join('; ')}` : '',
      ].filter(Boolean);
      return lines.length ? lines.join(' ') : 'I have not learned strong patterns yet. I will keep watching for what helps, and ask before turning patterns into routines.';
    },
    soundscape_companion: async ({ query, useCase, rememberPreference }) => {
      const plan = await resolveSoundscapeCompanion(String(query || 'calm ambient'), String(useCase || 'conversation'));
      if (!plan) return 'I could not create a local soundscape for that yet.';
      ctx.publish({ activeMedia: { ...plan, state: 'playing' }, mediaCommand: null, mediaVolume: plan.volume });
      ctx.setMode('media');
      await observeHabitSignal({ kind: 'music', label: plan.spokenLabel, context: { useCase: useCase || 'conversation', soundscape: plan.kind }, confidence: 0.62 });
      if (rememberPreference) await rememberSoundscapeUse(plan.kind, plan.spokenLabel, String(useCase || 'conversation'), plan.volume);
      await logEvent('soundscape.companion', `${plan.kind}: ${query || ''}`);
      await ctx.refresh();
      return `Starting ${plan.spokenLabel}, quietly under my voice.`;
    },
    start_guided_session: async ({ kind, goal, durationMinutes }) => {
      ...
      const cue = await startGuidedStatefulSession(preset.kind, goal ? String(goal) : preset.theme ?? null);
      if (!cue) return 'I could not start that guided session.';
      const instructions = [
        ...
        'State machine is active. Speak the current cue only, then wait when asked. Use guided_session_control next/repeat/skip/end to advance.',
      ].filter(Boolean).join('\n\n');
      const activeSession = {
        kind: 'remote' as SessionKind,
        label: preset.label,
        ...
        iconUrl: null,
        imageUrl: null,
        toolNames: ['guided_session_control', 'reflect_session', 'update_user_profile', 'get_user_profile', 'soundscape_companion'],
        startedAt: new Date().toISOString(),
      };
      await setPrefs({ activeSession } as Partial<Preferences>);
      ctx.publish({ sessionLabel: activeSession.label, guidedCue: cue });
      ctx.updateRealtimeSession();
      await observeHabitSignal({ kind: preset.kind, label: preset.label, context: { goal: goal || preset.theme, durationMinutes }, confidence: 0.64 });
      await logEvent('guided_session.start', `${preset.id}${goal ? ` goal=${goal}` : ''}`);
      return guidedCueToSpeech(guidedSessionOpening(preset), cue);
    },
    guided_session_control: async ({ command, userText }) => {
      const cue = await controlGuidedStatefulSession(command || 'next', userText ? String(userText) : null);
      const cmd = String(command ?? '').toLowerCase();
      if (cmd === 'end' || cue.completed) {
        await setPrefs({ activeSession: null } as Partial<Preferences>);
        ctx.publish({ sessionLabel: null, guidedCue: cue });
        ctx.updateRealtimeSession();
        await logEvent('guided_session.end', cue.sessionId || 'guided');
      } else {
        ctx.publish({ guidedCue: cue });
        await logEvent('guided_session.control', `${cmd || 'next'}:${cue.phase}`);
      }
      return cue.speech;
    },
    ...
    reflect_session: async ({ summary, technique, goal, emotionalPattern, nextRitual }) => {
      ...
      if (technique || emotionalPattern || nextRitual) await observeHabitSignal({ kind: technique ? String(technique) : 'reflection', label: String(summary || technique || emotionalPattern || 'guided reflection'), context: { goal, emotionalPattern, nextRitual }, confidence: 0.7 });
      await ctx.refresh();
