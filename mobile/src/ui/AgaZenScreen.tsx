...
export function AgaZenScreen() {
  ...
  const audioLevel = mode === 'speaking' ? 0.82 : interim ? 0.55 : mode === 'listening' ? 0.2 : 0;
  const status = error ? 'Needs attention' : mode === 'sleeping' || mode === 'listening' ? 'Listening for AGA' : mode;
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        ...
        <Link href="/settings" asChild>
          <Pressable style={styles.settingsButton} accessibilityRole="button" accessibilityLabel="Open settings">
      ...
      <Animated.View
        ...
        style={[
          ...
          {
            transform: [
              { translateY: avatarShift.interpolate({ inputRange: [0, 1], outputRange: [40, -86] }) },
              { scale: avatarShift.interpolate({ inputRange: [0, 1], outputRange: [1.08, 0.82] }) },
            ],
          },
        ]}
      >
      ...
      <View style={styles.feedShell}>
        <FlatList
          ...
          renderItem={({ item }) => <MessageBubble message={item} onReplay={item.role === 'assistant' ? replay : undefined} />}
          ListEmptyComponent={
            <View style={styles.emptyState}>
              <Text style={styles.emptyTitle}>Say “Hey AGA”</Text>
              <Text style={styles.emptyText}>Ask for advice, reminders, memory, translation, or YouTube.</Text>
...
const styles = StyleSheet.create({
  ...
  backgroundOrbOne: { position: 'absolute', width: 360, height: 360, borderRadius: 180, backgroundColor: 'rgba(103,232,249,0.13)', top: -90, right: -120 },
  backgroundOrbTwo: { position: 'absolute', width: 320, height: 320, borderRadius: 160, backgroundColor: 'rgba(167,139,250,0.12)', bottom: -120, left: -90 },
  header: { position: 'absolute', top: spacing.lg, left: spacing.lg, right: spacing.lg, zIndex: 40, flexDirection: 'row', alignItems: 'center', gap: spacing.sm },
  brandPill: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, padding: spacing.sm, paddingRight: spacing.md, borderRadius: radius.pill, backgroundColor: 'rgba(255,255,255,0.08)', borderWidth: 1, borderColor: colors.border },
  ...
  statusPill: { flex: 1, minWidth: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm, paddingHorizontal: spacing.md, height: 45, borderRadius: radius.pill, backgroundColor: 'rgba(255,255,255,0.07)', borderWidth: 1, borderColor: colors.border },
  ...
  settingsButton: { width: 45, height: 45, borderRadius: 22.5, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,255,255,0.08)', borderWidth: 1, borderColor: colors.border },
  settingsText: { color: colors.text, fontSize: 18 },
  avatarWrap: { position: 'absolute', top: '23%', left: 0, right: 0, zIndex: 5, alignItems: 'center' },
  interimPill: { position: 'absolute', top: '48%', alignSelf: 'center', maxWidth: '86%', paddingHorizontal: spacing.lg, paddingVertical: spacing.md, borderRadius: radius.xl, backgroundColor: 'rgba(103,232,249,0.13)', borderWidth: 1, borderColor: 'rgba(103,232,249,0.28)', zIndex: 15 },
  ...
  feedShell: { position: 'absolute', left: spacing.lg, right: spacing.lg, bottom: spacing.lg, top: '48%', zIndex: 10, borderRadius: radius.xl, backgroundColor: 'rgba(255,255,255,0.045)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', overflow: 'hidden' },
