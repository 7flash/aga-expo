...
function normalizeRemoteResult(data: any, query: string): YouTubeResult | null {
  ...
  return {
    ...
    thumbnailUrl: first?.thumbnailUrl ?? first?.thumbnail ?? first?.artworkUrl ?? `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`,
    url: first?.url ?? `https://www.youtube.com/watch?v=${videoId}`,
  };
}
...
export async function searchYouTube(query: string): Promise<YouTubeResult> {
  const remote = await searchRemoteYouTube(query).catch(() => null);
