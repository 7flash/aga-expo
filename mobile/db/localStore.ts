...
export async function resetAgaData(scope: ResetScope = 'everything') {
  ...
  await transaction(async () => {
    ...
    await run('DELETE FROM guided_session_state');
    await run('DELETE FROM routines');
    await run('DELETE FROM learned_skills WHERE source = ?', ['learned']);
    await run('DELETE FROM habit_observations');
    await run('DELETE FROM scenario_patterns');
    await run('DELETE FROM soundscape_preferences');
...
export async function getDiagnostics() {
  ...
  const guidedSessionStates = sqliteAvailable ? Number((await first<{ count: number }>('SELECT COUNT(*) AS count FROM guided_session_state'))?.count ?? 0) : 0;
  const habitObservations = sqliteAvailable ? Number((await first<{ count: number }>('SELECT COUNT(*) AS count FROM habit_observations'))?.count ?? 0) : 0;
  const scenarioPatterns = sqliteAvailable ? Number((await first<{ count: number }>('SELECT COUNT(*) AS count FROM scenario_patterns'))?.count ?? 0) : 0;
  const soundscapePreferences = sqliteAvailable ? Number((await first<{ count: number }>('SELECT COUNT(*) AS count FROM soundscape_preferences'))?.count ?? 0) : 0;
  return {
    messages,
    memories,
    pendingReminders,
...
export type GuidedSessionStoreState = {
  id: string;
  presetId: string;
  kind: string;
  label: string;
...
export async function saveGuidedSessionState(state: GuidedSessionStoreState) {
  await ensureReady();
  if (!sqliteAvailable) return state;
  await run(
    `INSERT OR REPLACE INTO guided_session_state
      (id, presetId, kind, label, goal, phase, segmentIndex, paused, depth, userResponsesJson, lastCue, safetyFlagsJson, startedAt, updatedAt, completedAt)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?)`,
    [
      state.id,
      state.presetId,
      ...
      state.goal ?? null,
      state.phase,
      Math.max(0, Number(state.segmentIndex) || 0),
      state.paused ? 1 : 0,
      Math.max(0, Number(state.depth) || 0),
      asJson(state.userResponses ?? []),
      state.lastCue ?? null,
      asJson(state.safetyFlags ?? []),
      state.startedAt || isoNow(),
      state.completedAt ?? null,
    ],
  );
  return loadGuidedSessionState(state.id);
}

function mapGuidedSessionState(row: any): GuidedSessionStoreState | null {
  if (!row) return null;
  return {
    id: String(row.id),
    ...
    goal: row.goal ?? null,
    phase: String(row.phase || 'orientation'),
    segmentIndex: Number(row.segmentIndex ?? 0),
    paused: bool(row.paused, false),
    depth: Number(row.depth ?? 0),
    userResponses: safeJson<string[]>(row.userResponsesJson, []),
    lastCue: row.lastCue ?? null,
    safetyFlags: safeJson<string[]>(row.safetyFlagsJson, []),
    startedAt: String(row.startedAt || isoNow()),
    updatedAt: String(row.updatedAt || isoNow()),
    completedAt: row.completedAt ?? null,
  };
}

export async function loadGuidedSessionState(id?: string | null) {
  await ensureReady();
  if (!sqliteAvailable) return null;
  const row = id
    ? await first<any>('SELECT * FROM guided_session_state WHERE id = ? LIMIT 1', [id])
    : await first<any>('SELECT * FROM guided_session_state WHERE completedAt IS NULL ORDER BY updatedAt DESC LIMIT 1');
  return mapGuidedSessionState(row);
}

export async function completeGuidedSessionState(id?: string | null) {
  await ensureReady();
  if (!sqliteAvailable) return;
  if (id) await run('UPDATE guided_session_state SET completedAt = CURRENT_TIMESTAMP, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [id]);
  else await run('UPDATE guided_session_state SET completedAt = CURRENT_TIMESTAMP, updatedAt = CURRENT_TIMESTAMP WHERE completedAt IS NULL');
}

export async function recordHabitObservation(input: {
  kind: string;
  label: string;
  context?: MaybeJson;
  confidence?: number;
  ...
  const label = String(input.label || kind).trim();
  if (!sqliteAvailable || !kind || !label) return null;
  await run(
    `INSERT INTO habit_observations (kind, label, contextJson, confidence, createdAt) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)`,
    [kind, label, input.context ? asJson(input.context) : null, Math.max(0, Math.min(1, Number(input.confidence ?? 0.55)))],
  );
  return first('SELECT * FROM habit_observations ORDER BY id DESC LIMIT 1');
}

export async function upsertScenarioPattern(input: {
  patternKey: string;
  label: string;
  trigger?: MaybeJson;
  action?: MaybeJson;
  confidence?: number;
  consentState?: 'observed' | 'proposed' | 'accepted' | 'dismissed';
}) {
  await ensureReady();
  if (!sqliteAvailable) return null;
  const patternKey = String(input.patternKey || input.label || '').trim().toLowerCase();
  const label = String(input.label || patternKey || 'Scenario').trim();
  if (!patternKey || !label) return null;
  await run(
    `INSERT INTO scenario_patterns
      (patternKey, label, triggerJson, actionJson, observationCount, confidence, consentState, lastObservedAt, createdAt, updatedAt)
     VALUES (?, ?, ?, ?, 1, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
     ON CONFLICT(patternKey) DO UPDATE SET
      label = excluded.label,
      triggerJson = COALESCE(excluded.triggerJson, scenario_patterns.triggerJson),
      actionJson = COALESCE(excluded.actionJson, scenario_patterns.actionJson),
      observationCount = scenario_patterns.observationCount + 1,
      confidence = MIN(0.95, MAX(scenario_patterns.confidence, excluded.confidence) + 0.05),
      consentState = CASE WHEN scenario_patterns.consentState = 'accepted' THEN 'accepted' ELSE excluded.consentState END,
      lastObservedAt = CURRENT_TIMESTAMP,
      updatedAt = CURRENT_TIMESTAMP`,
    [patternKey, label, input.trigger ? asJson(input.trigger) : null, input.action ? asJson(input.action) : null, Math.max(0, Math.min(1, Number(input.confidence ?? 0.5))), input.consentState || 'observed'],
  );
  return first('SELECT * FROM scenario_patterns WHERE patternKey = ? LIMIT 1', [patternKey]);
}

export async function listScenarioPatterns(limit = 8, consentState?: string) {
  await ensureReady();
  if (!sqliteAvailable) return [];
  if (consentState) return all('SELECT * FROM scenario_patterns WHERE consentState = ? ORDER BY confidence DESC, updatedAt DESC LIMIT ?', [consentState, limit]);
  return all('SELECT * FROM scenario_patterns ORDER BY confidence DESC, updatedAt DESC LIMIT ?', [limit]);
}

export async function acceptProposedRoutine(match?: string | number | null) {
  await ensureReady();
  if (!sqliteAvailable) return null;
  const value = String(match ?? '').trim();
  const row = value
    ? await first<any>(`SELECT * FROM routines WHERE consentState = 'proposed' AND (CAST(id AS TEXT) = ? OR lower(title) LIKE lower(?)) ORDER BY confidence DESC LIMIT 1`, [value, `%${value}%`])
    : await first<any>(`SELECT * FROM routines WHERE consentState = 'proposed' ORDER BY confidence DESC, updatedAt DESC LIMIT 1`);
  if (!row) return null;
  await run(`UPDATE routines SET consentState = 'accepted', updatedAt = CURRENT_TIMESTAMP WHERE id = ?`, [row.id]);
  return first('SELECT * FROM routines WHERE id = ? LIMIT 1', [row.id]);
}

export async function dismissProposedRoutine(match?: string | number | null) {
  await ensureReady();
  if (!sqliteAvailable) return null;
  const value = String(match ?? '').trim();
  const row = value
    ...
    : await first<any>(`SELECT * FROM routines WHERE consentState = 'proposed' ORDER BY confidence DESC, updatedAt DESC LIMIT 1`);
  if (!row) return null;
  await run(`UPDATE routines SET consentState = 'dismissed', enabled = 0, updatedAt = CURRENT_TIMESTAMP WHERE id = ?`, [row.id]);
  return row;
}

export async function listDueAcceptedRoutines(now = new Date(), limit = 3) {
  await ensureReady();
  if (!sqliteAvailable) return [];
  ...
  return all(
    `SELECT * FROM routines
      WHERE enabled = 1
        AND consentState = 'accepted'
        AND (timeOfDay IS NULL OR timeOfDay = 'any' OR timeOfDay = ?)
        AND (lastFiredAt IS NULL OR lastFiredAt < datetime('now', '-6 hours'))
      ORDER BY confidence DESC, updatedAt DESC LIMIT ?`,
    [hour, limit],
  );
}

export async function markRoutineFired(id: number | string) {
  await ensureReady();
  if (!sqliteAvailable) return;
  await run('UPDATE routines SET lastFiredAt = CURRENT_TIMESTAMP, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [id]);
}

export async function saveSoundscapePreference(input: {
  kind: string;
  label: string;
  useCase?: string | null;
  volume?: number;
  ...
  if (!sqliteAvailable) return null;
  await run(
    `INSERT INTO soundscape_preferences (kind, label, useCase, volume, confidence, createdAt, updatedAt)
     VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
    [String(input.kind || 'calm'), String(input.label || input.kind || 'soundscape'), input.useCase ?? null, Math.max(0, Math.min(100, Math.round(Number(input.volume ?? 42)))), Math.max(0, Math.min(1, Number(input.confidence ?? 0.6)))],
  );
  return first('SELECT * FROM soundscape_preferences ORDER BY id DESC LIMIT 1');
}

export async function listSoundscapePreferences(useCase?: string | null, limit = 5) {
  await ensureReady();
  if (!sqliteAvailable) return [];
  if (useCase) return all('SELECT * FROM soundscape_preferences WHERE lower(useCase) LIKE lower(?) ORDER BY confidence DESC, updatedAt DESC LIMIT ?', [`%${useCase}%`, limit]);
  return all('SELECT * FROM soundscape_preferences ORDER BY confidence DESC, updatedAt DESC LIMIT ?', [limit]);
}
