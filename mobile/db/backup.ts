...
export type StorageSummary = {
  ...
  guidedSessionStates: number;
  habitObservations: number;
  scenarioPatterns: number;
  soundscapePreferences: number;
  events: number;
  backupBytes: number;
};
...
const TABLES = [
  ...
  'routines',
  'learned_skills',
  'guided_session_state',
  'habit_observations',
  'scenario_patterns',
  'soundscape_preferences',
  'event_log',
] as const;
...
const ALLOWED_COLUMNS: Record<BackupTable, readonly string[]> = {
  ...
  guided_session_state: ['id', 'presetId', 'kind', 'label', 'goal', 'phase', 'segmentIndex', 'paused', 'depth', 'userResponsesJson', 'lastCue', 'safetyFlagsJson', 'startedAt', 'updatedAt', 'completedAt'],
  habit_observations: ['id', 'kind', 'label', 'contextJson', 'confidence', 'createdAt'],
  scenario_patterns: ['id', 'patternKey', 'label', 'triggerJson', 'actionJson', 'observationCount', 'confidence', 'consentState', 'lastObservedAt', 'createdAt', 'updatedAt'],
  soundscape_preferences: ['id', 'kind', 'label', 'useCase', 'volume', 'confidence', 'createdAt', 'updatedAt'],
...
export async function getStorageSummary(): Promise<StorageSummary> {
  const [conversations, messages, memories, reminders, mediaSessions, mediaQueue, favorites, translations, routines, reflections, learnedSkills, guidedSessionStates, habitObservations, scenarioPatterns, soundscapePreferences, events, databaseBytes] = await Promise.all([
    count('conversations'),
    count('messages'),
    count('memory_facts'),
    count('reminders'),
    ...
    count('guided_session_state'),
    count('habit_observations'),
    count('scenario_patterns'),
    count('soundscape_preferences'),
    count('event_log'),
