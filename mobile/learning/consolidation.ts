...
import { addMemory, listProposedRoutines, upsertLearnedRoutine, listScenarioPatterns } from '../db/localStore';
import { inferHabitRoutines } from './habitTracker';
...
export async function runLearningConsolidation(): Promise<ConsolidationResult> {
  const [eventRoutineProposals, promotedMemories, inferredHabitRoutines] = await Promise.all([
    proposeRoutinesFromEvents(),
    promoteReflectionsToMemory(),
    inferHabitRoutines(),
  ]);
  const scenarios = await listScenarioPatterns(12).catch(() => [] as any[]);
  const observedScenarios = scenarios.filter((row: any) => row.consentState === 'observed' && Number(row.observationCount || 0) >= 3).length;
  const proposedRoutines = eventRoutineProposals + inferredHabitRoutines;
  ...
  return {
    observed: proposedRoutines + promotedMemories + observedScenarios,
    proposedRoutines,
    promotedMemories,
    inferredHabitRoutines,
    observedScenarios,
    speech,
  };
}
